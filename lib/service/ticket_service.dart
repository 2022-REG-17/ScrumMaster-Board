...
class TicketService extends GetConnect {
  ...
  static Future<Ticket> getTickets(int page, int size) async {
    ...
    try {
      final response = await client.get(
        Uri.parse("http://192.168.8.102:8080/api/v1/tickets?page=" +
            page.toString() +
            "&size=" +
            size.toString() +
            "&sort=".toString()),
  ...
  Future<String> saveTicket(Map data) async {
    try
    {
      final response = await post(
          "http://192.168.8.102:8080/api/v1/tickets",data);
  ...
  //update
  Future<String> updateTicket(Map data, String id) async {
    try
    {
      final response = await put(
          "http://192.168.8.102:8080/api/v1/tickets/$id",data);
      if (response.status.hasError) {
        return Future.error(response.statusText);
      } else {
        return  response.body['result'];
  ...
  Future<String> deleteTicket(String id) async {
    try{
      final response = await delete(
          "http://192.168.8.102:8080/api/v1/tickets/$id");
      if (response.status.hasError) {
        return Future.error(response.statusText);
      } else {
        return response.body['result'];
